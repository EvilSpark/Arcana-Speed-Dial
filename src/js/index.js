...
const createDialModalContent = document.getElementById(
    "createDialModalContent"
);
...
const createFolderModalContent = document.getElementById(
...
const editFolderModalContent = document.getElementById(
...
const deleteFolderModalContent = document.getElementById(
...
function removeBookmark(url) {
    browser.bookmarks.search({ url }).then(bookmarks => {
        for (let bookmark of bookmarks) {
            if (
                bookmark.parentId === speedDialId ||
                folders.indexOf(bookmark.parentId) !== -1
            ) {
                targetNode.remove();
...
function getThumbs(bookmarkUrl) {
    return browser.storage.local.get(bookmarkUrl).then(result => {
...
function sort() {
    browser.storage.local.get(speedDialId).then(result => {
...
function printBookmarks(bookmarks, parentId) {
        ...
        for (let bookmark of bookmarks) {
            ...
            if (!bookmark.url && bookmark.dateGroupModified) {
                ...
                if (folders.indexOf(bookmark.id) === -1) {
                    ...
                    if (
                        bookmark.id === targetFolder &&
                        targetFolderName !== bookmark.title
                    ) {
    ...
    if (parentId === speedDialId) {
        ...
        // sort
        browser.storage.local.get(parentId).then(result => {
            if (result[parentId]) {
...
function saveBookmarkSettings() {
        ...
        browser.storage.local.get(url).then(result => {
            if (result[url]) {
                ...
                browser.storage.local
                    .set({ [url]: { thumbnails, thumbIndex } })
                    .then(result => {
                        tabMessagePort.postMessage({
                            updateCache: true,
                            url,
                            i: thumbIndex,
                        });
    ...
    if (title !== targetTileTitle) {
        ...
        browser.bookmarks.search({ url }).then(bookmark => {
...
document.addEventListener("contextmenu", function (e) {
    if (
        e.target.type === "text" &&
        (e.target.id === "modalTitle" || e.target.id === "createDialModalURL")
    ) {
        return;
    }
    e.preventDefault();
    ...
    if (e.target.className === "tile-content") {
        ...
        e.target.className === "tile folderTitle" &&
        e.target.id !== "homeFolderLink"
    ) {
        ...
        e.target.className === "folders" ||
        e.target.className === "container" ||
        e.target.className === "tileContainer" ||
        e.target.className === "default-content"
    ) {
...
window.addEventListener("click", e => {
    if (
        ...
        e.target.className.indexOf("settingsCtl") >= 0
    ) {
        return;
    }
    if (
        e.target.className === "tile-content" ||
        e.target.className === "tile-title"
    ) {
        return;
    }
    e.preventDefault();
